...
for i, (shoulder_rotation, shoulder_time) in enumerate(examples, 1):
    ...
    # 提取結果
    elbow_extension = y_pred[0, 0]
    elbow_time = y_pred[0, 1]
    
    print(f"示例 {i}:")
    print(f"  輸入 - 最大肩外旋: {shoulder_rotation:.1f}°, 時間: {shoulder_time:.2f}s")
    print(f"  預測 - 最大肘外展: {elbow_extension:.2f}°, 時間: {elbow_time:.4f}s")
    print()
...
while True:
    try:
        ...
        print(f"\n預測結果:")
        print(f"  最大肘外展: {elbow_extension:.2f}°")
        print(f"  發生時間: {elbow_time:.4f}s")
