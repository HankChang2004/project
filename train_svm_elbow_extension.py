"""
Train SVM model to predict max elbow extension and time from max shoulder external rotation and time
訓練SVM模型，輸入是最大肩外旋和時間，輸出是最大肘外展和時間
"""
...
# Output: max_elbow_extension, time_max_elbow_extension
X = df[['max_shoulder_external_rotation', 'time_max_shoulder_external_rotation']].values
y = df[['max_elbow_extension', 'time_max_elbow_extension']].values
...
output_names = ['max_elbow_extension', 'time_max_elbow_extension']
print("=" * 60)
print("模型評估結果")
print("=" * 60)
print(f"\n整體指標:")
...
print("\n正在保存模型...")
joblib.dump(best_model, 'svm_elbow_extension_model.pkl')
joblib.dump(scaler_X, 'scaler_X_elbow_extension.pkl')
joblib.dump(scaler_y, 'scaler_y_elbow_extension.pkl')
...
with open('svm_elbow_extension_model_summary.json', 'w', encoding='utf-8') as f:
    json.dump(model_summary, f, indent=2, ensure_ascii=False)

print("模型已保存:")
print("  - svm_elbow_extension_model.pkl")
print("  - scaler_X_elbow_extension.pkl")
print("  - scaler_y_elbow_extension.pkl")
print("  - svm_elbow_extension_model_summary.json")
...
print(f"{'(肩外旋, 時間)':<50} | {'(肘外展, 時間)':<35} | {'(肘外展, 時間)':<35}")
print("-" * 120)
